...
/*
 * Plugin Name: Hide wp-login.php
 * Plugin URI:  https://www.wpwebteach.bid/hide-wp-login-php
 * Description: Change wp-login.php to whatever you want When Ever You Want. Just Go To Setting > permalink > Scroll Down > Hide WP-Login.php Change Your Website URI.
 * Version:     1.0
 * Author:      Neel Patel
 * Author URI:  https://wpwebteach.top 
...
if ( defined( 'ABSPATH' ) && ! class_exists( 'Hide_WP_Login' ) ) {
	...
	class Hide_WP_Login {
		...
		public function __construct() {
			...
			register_uninstall_hook( $this->basename(), array( 'Hide_WP_Login', 'uninstall' ) );
		...
		public function admin_notices_incompatible() {
			echo '<div class="error"><p>' . sprintf( __( 'Please upgrade to the latest version of WordPress to activate %s.', 'Hide-wp-login' ), '<strong>' . __( 'Hide wp-login.php', 'Hide-wp-login' ) . '</strong>' ) . '</p></div>';
		}
		...
		public function wpmu_options() {
			...
			$out .= '<h3>' . __( 'Hide wp-login.php', 'Hide-wp-login' ) . '</h3>';
			$out .= '<p>' . __( 'This option allows you to set a networkwide default, which can be overridden by individual sites. Simply go to to the site’s permalink settings to change the url.', 'Hide-wp-login' ) . '</p>';
			$out .= '<table class="form-table">';
				$out .= '<tr valign="top">';
					$out .= '<th scope="row">' . __( 'Networkwide default', 'Hide-wp-login' ) . '</th>';
		...
		public function admin_init() {
			...
			add_settings_section(
				'Hide-wp-login-section',
				__( 'Hide wp-login.php', 'Hide-wp-login' ),
			...
			add_settings_field(
				'rwl-page',
				'<label for="rwl-page">' . __( 'Login url', 'Hide-wp-login' ) . '</label>',
		...
		public function rwl_section_desc() {
			...
			if ( is_multisite() && is_super_admin() && is_plugin_active_for_network( $this->basename() ) ) {
				$out .= '<p>' . sprintf( __( 'To set a networkwide default, go to %s.', 'Hide-wp-login' ), '<a href="' . network_admin_url( 'settings.php#rwl-page-input' ) . '">' . __( 'Network Settings', 'Hide-wp-login' ) . '</a>') . '</p>';
			}
		...
		public function admin_notices() {
			...
			if ( ! is_network_admin() && $pagenow === 'options-permalink.php' && isset( $_GET['settings-updated'] ) ) {
				echo '<div class="updated"><p>' . sprintf( __( 'Your login page is now here: %s. Bookmark this page!', 'Hide-wp-login' ), '<strong><a href="' . $this->new_login_url() . '">' . $this->new_login_url() . '</a></strong>' ) . '</p></div>';
			}
		}

		public function plugin_action_links( $links ) {
			if ( is_network_admin() && is_plugin_active_for_network( $this->basename() ) ) {
				array_unshift( $links, '<a href="' . network_admin_url( 'settings.php#rwl-page-input' ) . '">' . __( 'Settings', 'Hide-wp-login' ) . '</a>' );
			} elseif ( ! is_network_admin() ) {
				array_unshift( $links, '<a href="' . admin_url( 'options-permalink.php#rwl-page-input' ) . '">' . __( 'Settings', 'Hide-wp-login' ) . '</a>' );
			}
		...
		public function wp_loaded() {
			...
			if ( is_admin() && ! is_user_logged_in() && ! defined( 'DOING_AJAX' ) ) {
				wp_die( __( 'You must log in to access the admin area.', 'Hide-wp-login' ) );
			}
